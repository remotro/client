...
use remotro::{
    balatro::{
        boosters::{Open, OpenBoosterPack, OpenWithHand},
        hud::{Hud, RunInfo},
        menu::{Deck, Stake},
...
fn display_menu() {
    ...
    println!(
        "  {} - Continue saved run (if available)",
        "continue".yellow().bold()
    );
}
...
fn display_blinds_menu() {
    ...
    println!("{}", "Available actions:".white());
    println!(
        "  {} - Select and play the current blind",
        "select".green().bold()
    );
...
fn display_play_menu() {
    ...
    println!("{}", "Available actions:".white());
    println!(
        "  {} - Select/deselect cards (e.g., 'select 0 1 2')",
        "select <indices>".cyan().bold()
    );
    println!(
        "  {} - Rearrange cards in hand",
        "move <from> <to>".yellow().bold()
    );
...
fn display_shop_menu() {
    ...
    println!("{}", "Available actions:".white());
    println!(
        "  {} - Buy item (types: main, voucher, booster)",
        "buy <type> <index>".cyan().bold()
    );
...
fn display_pack_menu() {
    ...
    println!(
        ...
        "select <index>".cyan().bold()
    );
    println!(
        "  {} - Click cards in hand (for packs with hands)",
        "click <indices>".cyan().bold()
    );
    println!(
        "  {} - Move cards around (for packs with hands)",
        "move <from> <to>".yellow().bold()
    );
...
fn display_joker_menu() {
    ...
    println!("{}", "Available actions:".white());
    println!(
        "  {} - Move joker from position to position",
        "move <from> <to>".cyan().bold()
    );
...
fn display_consumable_menu() {
    ...
    println!(
        ...
        "use <index>".green().bold()
    );
    println!(
        "  {} - Move consumable from position to position",
        "move <from> <to>".cyan().bold()
    );
    println!(
        "  {} - Sell consumable at index",
        "sell <index>".red().bold()
    );
    println!("  {} - Return to HUD menu", "back".blue().bold());
}
...
fn print_run_info(run_info: &RunInfo) {
    ...
    if !run_info.vouchers_redeemed.is_empty() {
        println!(
            "{}: {:?}",
            "Vouchers Redeemed".cyan().bold(),
            run_info.vouchers_redeemed
        );
    }
    ...
    if hands.high_card.played > 0 {
        println!(
            "  High Card: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.high_card.hand.level,
            hands.high_card.hand.chips,
            hands.high_card.hand.mult,
            hands.high_card.played
        );
    }
    if hands.pair.played > 0 {
        println!(
            "  Pair: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.pair.hand.level, hands.pair.hand.chips, hands.pair.hand.mult, hands.pair.played
        );
    }
    if hands.two_pair.played > 0 {
        println!(
            "  Two Pair: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.two_pair.hand.level,
            hands.two_pair.hand.chips,
            hands.two_pair.hand.mult,
            hands.two_pair.played
        );
    }
    if hands.three_of_a_kind.played > 0 {
        println!(
            "  Three of a Kind: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.three_of_a_kind.hand.level,
            hands.three_of_a_kind.hand.chips,
            hands.three_of_a_kind.hand.mult,
            hands.three_of_a_kind.played
        );
    }
    if hands.straight.played > 0 {
        println!(
            "  Straight: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.straight.hand.level,
            hands.straight.hand.chips,
            hands.straight.hand.mult,
            hands.straight.played
        );
    }
    if hands.flush.played > 0 {
        println!(
            "  Flush: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.flush.hand.level,
            hands.flush.hand.chips,
            hands.flush.hand.mult,
            hands.flush.played
        );
    }
    if hands.full_house.played > 0 {
        println!(
            "  Full House: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.full_house.hand.level,
            hands.full_house.hand.chips,
            hands.full_house.hand.mult,
            hands.full_house.played
        );
    }
    if hands.four_of_a_kind.played > 0 {
        println!(
            "  Four of a Kind: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.four_of_a_kind.hand.level,
            hands.four_of_a_kind.hand.chips,
            hands.four_of_a_kind.hand.mult,
            hands.four_of_a_kind.played
        );
    }
    if hands.straight_flush.played > 0 {
        println!(
            "  Straight Flush: Lv.{} ({} chips, {}x mult) - Played {} times",
            hands.straight_flush.hand.level,
            hands.straight_flush.hand.chips,
            hands.straight_flush.hand.mult,
            hands.straight_flush.played
        );
    }
    ...
    if let Some(five_kind) = &hands.five_of_a_kind {
        if five_kind.played > 0 {
            println!(
                "  Five of a Kind: Lv.{} ({} chips, {}x mult) - Played {} times",
                five_kind.hand.level, five_kind.hand.chips, five_kind.hand.mult, five_kind.played
            );
        }
    }
    if let Some(flush_house) = &hands.flush_house {
        if flush_house.played > 0 {
            println!(
                "  Flush House: Lv.{} ({} chips, {}x mult) - Played {} times",
                flush_house.hand.level,
                flush_house.hand.chips,
                flush_house.hand.mult,
                flush_house.played
            );
        }
    }
    if let Some(flush_fives) = &hands.flush_fives {
        if flush_fives.played > 0 {
            println!(
                "  Flush Fives: Lv.{} ({} chips, {}x mult) - Played {} times",
                flush_fives.hand.level,
                flush_fives.hand.chips,
                flush_fives.hand.mult,
                flush_fives.played
            );
        }
    }
...
async fn handle_hud_management<'a, T: Hud<'a>>(
    mut screen: T,
...
async fn handle_joker_management<'a, T: Hud<'a>>(
    mut screen: T,
) -> Result<T, Box<dyn std::error::Error>> {
    loop {
        display_joker_menu();
...
async fn handle_consumable_management<'a, T: Hud<'a>>(
    mut screen: T,
) -> Result<T, Box<dyn std::error::Error>> {
    loop {
        ...
        screen = match parts.get(0).map(|s| s.to_lowercase()).as_deref() {
            ...
            _ => {
                println!(
                    "Invalid action. Use 'use <index>', 'move <from> <to>', 'sell <index>', or 'back'."
                );
                screen
            }
        };
            ...
            match balatro.screen().await {
                Ok(screen) => match screen {
                    CurrentScreen::Menu(menu) => {
                        ...
                        match action.trim().to_lowercase().as_str() {
                            "new" => {
                                let deck: Deck = get_input(
                                    "Select Deck (Red/Blue/Yellow/Green/Black/Magic/Nebula/Ghost/Abandoned/Checkered/Zodiac/Painted/Anaglyph/Plasma/Erratic):",
                                );
                                let stake: Stake = get_input(
                                    "Select Stake (White/Red/Green/Blue/Black/Purple/Orange/Gold):",
                                );
                    ...
                    CurrentScreen::RoundOverview(overview) => {
                        ...
                        println!(
                            "{}: ${}",
                            "Total Earned".bright_yellow().bold(),
                            overview.total_earned()
                        );
                        println!(
                            "{}: {:?}",
                            "Earnings Breakdown".cyan().bold(),
                            overview.earnings()
                        );

                        let action = get_string_input("Enter action:");
                        match action.trim().to_lowercase().as_str() {
                            "continue" => match overview.cash_out().await {
                    ...
                    CurrentScreen::GameOver(game_over) => {
                        ...
                        println!(
                            "{}: {:?}",
                            "Most Played Hand".green().bold(),
                            game_over.most_played_hand()
                        );
                    ...
                    CurrentScreen::Play(play) => {
                        ...
                        for (i, hand_card) in play.hand().iter().enumerate() {
                            if let Some(card) = &hand_card.card {
                                let selection = if hand_card.selected {
                                ...
                                if let Some(enhancement) = &card.enhancement {
                                    card_parts.push(
                                        format!("{:?}", enhancement).yellow().bold().to_string(),
                                    );
                                }
                                ...
                                if let Some(edition) = &card.edition {
                                    card_parts.push(
                                        format!("{:?}", edition).bright_cyan().bold().to_string(),
                                    );
                                }
                                ...
                                if let Some(seal) = &card.seal {
                                    card_parts.push(
                                        format!("{:?} Seal", seal).magenta().bold().to_string(),
                                    );
                                }
                                ...
                                println!(
                                    ...
                                    i,
                                    selection,
                                    "[Empty Card Slot]".bright_black().italic()
                                );
                            }
                        }
                        for (i, card) in play.deck().iter().enumerate() {
                            println!(
                                "{i} : {:?} of {:?} ({})",
                                card.rank, card.suit, card.debuffed
                            )
                        }
                        for (i, card) in play.discarded().iter().enumerate() {
                            println!(
                                "{i} : {:?} of {:?} ({})",
                                card.rank, card.suit, card.debuffed
                            )
                        ...
                        match parts.get(0).map(|s| s.to_lowercase()).as_deref() {
                            Some("select") => {
                                if parts.len() > 1 {
                                    let indices: Result<Vec<u32>, _> =
                                        parts[1..].iter().map(|s| s.parse()).collect();
                                    match indices {
                                        Ok(indices) => match play.click(&indices).await {
                                            Ok(_) => println!("Cards selected successfully"),
                                            Err(e) => error!("Failed to select cards: {}", e),
                                        },
                                        Err(_) => println!(
                                            "Invalid card indices. Use numbers separated by spaces."
                                        ),
                                    }
                                } else {
                            ...
                            Some("play") => match play.play().await {
                            ...
                            Some("discard") => match play.discard().await {
                            ...
                            Some("hud") => match handle_hud_management(play).await {
                                Ok(_updated_play) => {
                                    println!("Returned from HUD management");
                                }
                                Err(e) => error!("HUD management error: {}", e),
                            ...
                            _ => println!(
                                "Invalid action. Use 'select <indices>', 'play', 'discard', or 'hud'."
                            ),
                        }
                    }
                    CurrentScreen::Shop(shop) => {
                        ...
                        match parts.get(0).map(|s| s.to_lowercase()).as_deref() {
                            ...
                            _ => println!(
                                "Invalid action. Use 'buy <type> <index>', 'reroll', 'leave', or 'hud'."
                            ),
                        }
                    }
                        ...
                        match pack {
                            OpenBoosterPack::Arcana(pack) => {
                                ...
                                for (i, card) in hand.iter().enumerate() {
                                    let selection = if card.selected {
                                        "[SELECTED]"
                                    } else {
                                        "         "
                                    };
                                    println!(
                                        "  {}: {} {:?} of {:?}",
                                        i, selection, card.card.rank, card.card.suit
                                    );
                                ...
                                match parts.get(0).map(|s| s.to_lowercase()).as_deref() {
                                    Some("select") => {
                                        if parts.len() == 2 {
                                            ...
                                            println!(
                                                "Please specify option index (e.g., 'select 0')"
                                            );
                                        }
                                    }
                                    Some("click") => {
                                        if parts.len() > 1 {
                                            let indices: Result<Vec<u32>, _> =
                                                parts[1..].iter().map(|s| s.parse()).collect();
                                            match indices {
                                                Ok(indices) => match pack.click(&indices).await {
                                                    Ok(_) => println!("Cards clicked successfully"),
                                                    Err(e) => {
                                                        error!("Failed to click cards: {}", e)
                                                    }
                                                },
                                            ...
                                            println!(
                                                "Please specify card indices (e.g., 'click 0 1')"
                                            );
                                        }
                                    }
                                    Some("move") => {
                                    ...
                                    Some("skip") => match pack.skip().await {
                                        Ok(_) => println!("Skipped selection"),
                                        Err(e) => error!("Failed to skip: {}", e),
                                    },
                                    Some("hud") => match handle_hud_management(pack).await {
                                        Ok(_updated_pack) => {
                                            println!("Returned from HUD management");
                                        }
                                        Err(e) => error!("HUD management error: {}", e),
                                    ...
                                    _ => println!(
                                        "Invalid action. Use 'select <index>', 'click <indices>', 'skip', or 'hud'."
                                    ),
                                }
                            }
                            OpenBoosterPack::Buffoon(pack) => {
                                ...
                                match parts.get(0).map(|s| s.to_lowercase()).as_deref() {
                                    Some("select") => {
                                        if parts.len() == 2 {
                                            if let Ok(index) = parts[1].parse::<u32>() {
                                                match pack.select(index).await {
                                                    Ok(_) => {
                                                        println!("Joker selected successfully")
                                                    }
                                                    Err(e) => {
                                                        error!("Failed to select joker: {}", e)
                                                    }
                                                }
                                            } else {
                                                println!("Invalid index. Use a number.");
                                            ...
                                            println!(
                                                "Please specify joker index (e.g., 'select 0')"
                                            );
                                        }
                                    }
                                    Some("skip") => match pack.skip().await {
                                    ...
                                    _ => println!(
                                        "Invalid action. Use 'select <index>', 'skip', or 'hud'."
                                    ),
                                }
                            }
                                    ...
                                    Some("select") => {
                                        if parts.len() == 2 {
                                            if let Ok(index) = parts[1].parse::<u32>() {
                                                match pack.select(index).await {
                                                    Ok(_) => {
                                                        println!("Planet selected successfully")
                                                    }
                                                    Err(e) => {
                                                        error!("Failed to select planet: {}", e)
                                                    }
                                                }
                                            } else {
                                                println!("Invalid index. Use a number.");
                                            ...
                                            println!(
                                                "Please specify planet index (e.g., 'select 0')"
                                            );
                                        }
                                    }
                                    Some("skip") => match pack.skip().await {
                                ...
                                match parts.get(0).map(|s| s.to_lowercase()).as_deref() {
                                    Some("select") => {
                                        if parts.len() == 2 {
                                            ...
                                            println!(
                                                "Please specify card index (e.g., 'select 0')"
                                            );
                                        }
                                    }
                                    Some("skip") => match pack.skip().await {
