...
impl Translatable for JokerKind {
    fn translate(&self, translations: &Translations) -> Translation {
        ...
        match self {
            ...
            Self::GreedyJoker => render!(translations, path, 3, "Diamond").unwrap(),
            Self::LustyJoker => render!(translations, path, 3, "Heart").unwrap(),
            Self::WrathfulJoker => render!(translations, path, 3, "Spade").unwrap(),
            Self::GluttenousJoker => render!(translations, path, 3, "Club").unwrap(),
            Self::Jolly => render!(translations, path, 8, "Pair").unwrap(),
            Self::Zany => render!(translations, path, 12, "Three of a Kind").unwrap(),
            Self::Mad => render!(translations, path, 10, "Two Pair").unwrap(),
            Self::Crazy => render!(translations, path, 12, "Straight").unwrap(),
            Self::Droll => render!(translations, path, 10, "Flush").unwrap(),
            Self::Sly => render!(translations, path, 50, "Pair").unwrap(),
            Self::Wily => render!(translations, path, 100, "Three of a Kind").unwrap(),
            Self::Clever => render!(translations, path, 80, "Two Pair").unwrap(),
            Self::Devious => render!(translations, path, 100, "Straight").unwrap(),
            Self::Crafty => render!(translations, path, 80, "Flush").unwrap(),
            Self::Half => render!(translations, path, 20, 3).unwrap(),
            ...
            Self::MysticSummit => render!(translations, path, 15, 0).unwrap(),
            ...
            Self::Constellation { xmult } => render!(translations, path, 0.01, *xmult).unwrap(),
            Self::Hiker => render!(translations, path, 5).unwrap(),
            Self::Faceless => render!(translations, path, 5, 3).unwrap(),
            Self::GreenJoker { mult } => render!(translations, path, 1, 1, *mult).unwrap(),
            Self::Superposition => render!(translations, path).unwrap(),
            Self::TodoList { poker_hand } => render!(translations, path, 4, poker_hand.translate(translations).name).unwrap(),
            Self::Cavendish { probability } => render!(translations, path, 3, *probability, 1000).unwrap(),
            Self::CardSharp => render!(translations, path, 3).unwrap(),
            Self::RedCard { mult } => render!(translations, path, 3, *mult).unwrap(),
            Self::Madness { xmult } => render!(translations, path, 0.5, *xmult).unwrap(),
            Self::Square { chips } => render!(translations, path, *chips, 4).unwrap(),
            Self::Seance => render!(translations, path, "Straight Flush").unwrap(),
            Self::RiffRaff => render!(translations, path, 2).unwrap(),
            Self::Vampire { xmult } => render!(translations, path, 0.1, *xmult).unwrap(),
            Self::Shortcut => render!(translations, path).unwrap(),
            Self::Hologram { xmult } => render!(translations, path, 0.25, *xmult).unwrap(),
            Self::Vagabond => render!(translations, path, 4).unwrap(),
            Self::Baron => render!(translations, path, 1.5).unwrap(),
            ...
            Self::Obelisk { xmult } => render!(translations, path, 0.2, *xmult).unwrap(), // CONFIRM
            ...
            Self::Erosion { mult } => render!(translations, path, 4, *mult, 52).unwrap(),
            Self::ReservedParking { probability } => render!(translations, path, 1,  *probability, 2).unwrap(),
            Self::Mail { rank } => render!(translations, path, 5, rank.translate(translations).name).unwrap(),
            ...
            Self::Stone { chips } => render!(translations, path, 25, *chips).unwrap(),
            Self::Golden => render!(translations, path, 4).unwrap(),
            Self::LuckyCat { xmult } => render!(translations, path, 0.25, *xmult).unwrap(),
            Self::Baseball => render!(translations, path, 1.5).unwrap(),
            Self::Bull { chips } => render!(translations, path, 2, *chips).unwrap(),
            Self::DietCola => render!(translations, path, "Double Tag").unwrap(),
            ...
            Self::Trousers { mult } => render!(translations, path, 2, "Two Pair", *mult).unwrap(),
            ...
            Self::Castle { chips, suit } => render!(translations, path, 3, suit.translate(translations).name, *chips).unwrap(),
            Self::Smiley => render!(translations, path, 5).unwrap(),
            Self::Campfire { xmult } => render!(translations, path, 0.25, *xmult).unwrap(),
            Self::Ticket => render!(translations, path, 4).unwrap(),
            ...
            Self::Troubadour => render!(translations, path, 2, 1).unwrap(),
            ...
            Self::HangingChad => render!(translations, path, 2).unwrap(),
            Self::RoughGem => render!(translations, path, 1).unwrap(),
            Self::Bloodstone { probability } => render!(translations, path, *probability, 2, 1.5).unwrap(),
            Self::Arrowhead => render!(translations, path, 50).unwrap(),
            Self::OnyxAgate => render!(translations, path, 7).unwrap(),
            Self::Glass { xmult  } => render!(translations, path, 0.75, *xmult).unwrap(),
            ...
            Self::Wee { chips } => render!(translations, path, *chips, 8).unwrap(),
            Self::MerryAndy => render!(translations, path, 3, -1).unwrap(),
            Self::Oops => render!(translations, path).unwrap(),
            Self::Idol { rank, suit } => render!(translations, path, 2, rank.translate(translations).name, suit.translate(translations).name + "s").unwrap(),
            Self::SeeingDouble => render!(translations, path, 2).unwrap(),
            Self::Matador => render!(translations, path, 8).unwrap(),
            Self::HitTheRoad { xmult } => render!(translations, path, 0.5, *xmult).unwrap(),
            Self::Duo => render!(translations, path, 2, "Pair").unwrap(),
            Self::Trio => render!(translations, path, 3, "Three of a Kind").unwrap(),
            Self::Family => render!(translations, path, 4, "Four of a Kind").unwrap(),
            Self::Order => render!(translations, path, 3, "Straight").unwrap(),
            Self::Tribe => render!(translations, path, 2, "Flush").unwrap(),
            Self::Stuntman => render!(translations, path, 250, 2).unwrap(),
            Self::Invisible { rounds } => render!(translations, path, 2, *rounds, 2).unwrap(),
            ...
            Self::DriversLicense { cards } => render!(translations, path, 3, *cards).unwrap(),
            ...
            Self::Yorick { left, xmult } => render!(translations, path, 1, 23, *left, *xmult).unwrap(),
